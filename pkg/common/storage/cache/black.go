...
type BlackCache interface {
	...
	// DelMaxBlackVersion is delete max black version
	DelMaxBlackVersion(ownerUserIDs ...string) BlackCache
	// FindMaxBlackVersionCache is find blacklist max version in cache.
	FindMaxBlackVersion(ctx context.Context, ownerUserID string) (*model.VersionLog, error)
}
